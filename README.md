...
이 저장소는 기능 플래그 관리와 관련된 두 가지 프로젝트를 포함합니다:

1.  **`feature_flag`**: Kotlin 및 Spring Boot 프레임워크를 사용하여 개발된 기능 플래그 관리 시스템입니다. 코드 배포 없이 기능 릴리스를 동적으로 제어할 수 있습니다.

    - 자세한 내용은 [feature_flag README](./feature_flag/README.md)를 참조하십시오.

2.  **`flag_speckit`**: Kotlin 및 Spring Boot로 개발된 백엔드 서비스와 API와 상호 작용하기 위한 간단한 프론트엔드로 구성된 포괄적인 기능 플래그 관리 시스템입니다. 이 시스템은 기본 토글, 백분율 기반 롤아웃 및 속성 기반 타겟팅을 포함한 동적 기능 관리를 허용합니다.

    - 자세한 내용은 [flag_speckit README](./flag_speckit/README.md)를 참조하십시오.

## 프로젝트 개요

### `feature_flag`

이 프로젝트는 기능 릴리스를 제어하는 데 필요한 필수 기능을 제공하는 경량 기능 플래그 관리 시스템입니다. 기능 플래그의 기본 개념을 이해하기 위한 좋은 시작점입니다.

**1. 프로젝트 개요**

이 프로젝트는 Kotlin과 Spring Boot 프레임워크를 사용하여 개발된 기능 플래그 관리 시스템입니다. 코드 배포 없이 기능 릴리스를 동적으로 제어할 수 있습니다. 이를 통해 개발자는 실시간으로 기능을 켜거나 끌 수 있어 카나리 릴리스, A/B 테스트 및 점진적 롤아웃을 용이하게 합니다.

**2. 아키텍처**

이 프로젝트는 관심사의 명확한 분리를 촉진하고 유지 보수성을 향상시키는 계층형 아키텍처를 따릅니다.
...
-   백분율 기반 롤아웃
-   속성 기반 타겟팅
-   기능 플래그 관리를 위한 간단한 프론트엔드

**1. 프로젝트 개요**

이 프로젝트는 기능 릴리스를 제어하기 위한 포괄적인 솔루션을 제공하도록 설계된 기능 플래그 관리 시스템입니다. Kotlin 및 Spring Boot로 개발된 백엔드 서비스와 API와 상호 작용하기 위한 간단한 프론트엔드로 구성됩니다. 이 시스템은 기본 토글, 백분율 기반 롤아웃 및 속성 기반 타겟팅을 포함한 동적 기능 관리를 허용합니다.

**2. 아키텍처**

이 프로젝트는 백엔드와 프론트엔드 간의 명확한 분리를 통해 표준 웹 애플리케이션 아키텍처를 따릅니다.
...
프론트엔드는 HTML 및 바닐라 JavaScript로 구축된 간단한 단일 페이지 애플리케이션(SPA)입니다. 백엔드와 통신하여 기능 플래그를 표시하고 관리합니다.
