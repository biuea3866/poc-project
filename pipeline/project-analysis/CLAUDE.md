...
| 에이전트 | 역할 | 전문성 | 산출물 |
|---------|------|--------|--------|
| `prd-functional` | 기능 요구사항 분석가 | Happy/Unhappy path, 역할별 동작 차이, 누락 기능 발견 | `*_기능요구사항.md` |
| `prd-nonfunctional` | 비기능 요구사항 분석가 | 성능, 보안, 확장성, 마이그레이션, 멱등성 | `*_비기능요구사항.md` |
| `prd-ambiguity` | 모호성 검출 전문가 | 용어 불일치, 조건 누락, 범위 모호성 → PM 질문 목록 | `*_모호성.md` |
| `prd-feasibility` | 기술 실현성 분석가 | 영향 서비스 식별, 코드 패턴 확인, DB/Kafka/인증 변경, 리스크 | `*_기술실현성.md` |
| `prd-scope` | 작업 범위 산정가 | FE/BE/인프라 변경 범위, QA 시나리오, 타 팀 협업 | `*_작업범위.md` |
| `prd-fe-dependency` | FE 의존성 분석가 | FE 레포 전수 조사, API 호출 매핑, JSON 호환성, Swagger | `*_FE의존성.md` |
| `prd-routing` | 라우팅/인프라 분석가 | Gateway 라우팅, Nginx Ingress, 인증 필터, CORS, K8s | `*_라우팅분석.md` |
| `prd-domain-arch` | 도메인 아키텍처 분석가 | 현재 도메인 구조 파악, 데이터 모델/이벤트/배치 | `*_도메인아키텍처.md` |
| `prd-migration` | 데이터 마이그레이션 전문가 | 현재 데이터 구조, 이관 계획, 하위 호환성, 롤백 | `*_데이터마이그레이션.md` |

### Phase 1-2.5: 리뷰 게이트 에이전트

| 에이전트 | 역할 | 전문성 |
|---------|------|--------|
| `review-completeness` | 산출물 완전성 검증자 | 에이전트 산출물 빈 파일 없는지, 누락 에이전트 없는지, 코드 근거 있는지, 상호 모순 없는지 |

### Phase 1.5: 벤치마킹 에이전트

| 에이전트 | 역할 | 전문성 | 산출물 |
|---------|------|--------|--------|
| `bench-researcher` | 벤치마킹 리서처 | 경쟁사/참조 SaaS/인프라 제품 조사, 패턴 분류, 자사 제품 시사점 | `*_benchmarking.md` |
| `bench-architect` | 아키텍처 설계자 | 벤치마킹 기반 통합 설계, 채택/미채택 결정, 추상화 우선 | `*_architecture.md` |

### Phase 2: BE 구현 설계 에이전트

| 에이전트 | 역할 | 전문성 | 산출물 |
|---------|------|--------|--------|
| `tech-arch` | 아키텍처 분석가 | Hexagonal 레이어, API 패턴, Kafka 토폴로지, 공유 라이브러리 | `architecture_analysis.md` |
| `tech-data` | 데이터 분석가 | MySQL 스키마, Flyway, MongoDB, Redis, 인덱스/쿼리 패턴 | `data_analysis.md` |
| `ticket-splitter` | 티켓 분할 전문가 | 독립 배포 단위 분할, 레이어별 분리, 의존 관계 도출 | `tickets/` |
| `test-designer` | 테스트 설계자 | 티켓별 TC Given/When/Then, Unit/Integration/E2E 레벨 | 각 티켓 TC |

### Phase 2-2.5: TDD 리뷰 게이트 에이전트

| 에이전트 | 역할 | 전문성 |
|---------|------|--------|
| `review-tdd` | TDD 품질 검증자 | 클래스 역할, Possible Solutions 풀어쓰기, 다이어그램, ERD, FE 영향, PRD 커버, Release |

### Phase 3: 구현 에이전트

| 에이전트 | 역할 | 전문성 |
|---------|------|--------|
| `impl-developer` | 구현 개발자 | TDD 사이클 (RED→GREEN), 티켓 단위 코드 작성, detekt 통과 |
| `impl-reviewer` | 코드 리뷰어 | 컨벤션 체크 (nullable, 캡슐화, 포트 우회, @Comment), 코드 중복, 네이밍 |
| `impl-prd-checker` | PRD 대조 검증자 | PRD vs 구현 Gap 체크, 메시지 템플릿/필드 불일치, 누락 기능 |
| `impl-doc-sync` | 문서 동기화 담당 | 구현 후 detailed_design/tdd/티켓 문서 업데이트, 의사결정 소급 반영 |
...
4. **리뷰 에이전트(impl-reviewer, impl-prd-checker)는 구현 묶음 완료 후** 스폰한다
...
|---------|-----------|---------|--------|
| **기능 요구사항** | 항상 | 기능별 완전성(입력→처리→출력), Happy/Unhappy path, 역할별 동작 차이, 기존 기능 관계(대체/확장/병존), 우선순위(MVP/후속), 누락 기능 | `{날짜}_{기능명}_기능요구사항.md` |
| **비기능 요구사항** | 항상 | 성능(응답시간/처리량/동시사용자), 보안(인증/인가/개인정보), 확장성(데이터증가/멀티테넌시), 다국어, 데이터 마이그레이션 전략, 롤백 시나리오, 멱등성 | `{날짜}_{기능명}_비기능요구사항.md` |
| **모호성/일관성** | 항상 | 용어 불일치, 기능 간 충돌, 범위 모호성, 조건 누락, 수치/제한값 미정의, 권한/역할별 동작 미명시, PRD 내 미결정 사항 → "PM에게 확인 필요" 질문 목록 | `{날짜}_{기능명}_모호성.md` |
| **기술 실현성** | 항상 | 영향 서비스 식별, 기존 코드 패턴 확인(코드 탐색 필수), DB 스키마 변경, Kafka 이벤트 변경, 인증/인가 변경, 기술 리스크 + 대안 | `{날짜}_{기능명}_기술실현성.md` |
| **FE/BE 작업범위** | 항상 | FE 변경 범위(화면/컴포넌트/API호출), BE 변경 범위(서비스/API/DB/이벤트), 인프라 변경, 디자인 산출물 필요 여부, QA 시나리오, 타 팀 협업 필요 여부 | `{날짜}_{기능명}_작업범위.md` |
| **FE 의존성** | API 변경/이관 시 | 대상 API를 호출하는 FE 레포 전수 조사(코드 탐색 필수), 엔드포인트별 호출 FE 매핑, 요청/응답 구조(FE가 의존하는 필드), 인증 방식, API 호스트 환경변수, JSON 직렬화 호환성(snake_case, null, Date 포맷), Swagger/OpenAPI 자동생성 영향 | `{날짜}_{기능명}_FE의존성.md` |
| **라우팅/인프라** | API 변경/이관 시 | API Gateway 라우팅 규칙(코드 탐색 필수), Nginx Ingress 경로 매핑, 인증 필터 체인(JWT→헤더 변환), 플랜 체크 적용 여부, CORS 설정, FE→Gateway→서비스 요청 흐름, K8s Service/Deployment 구조, 포팅 시 라우팅 변경 필요 여부 | `{날짜}_{기능명}_라우팅분석.md` |
| **도메인 아키텍처** | 기존 시스템 변경 시 | 현재 도메인 아키텍처 파악(코드 탐색 필수), 데이터 모델/이벤트 흐름/배치잡, 확장 설계 방향 제시(2차/3차 고도화 고려), 신규 테이블/이벤트/배치 설계 제안 | `{날짜}_{기능명}_도메인아키텍처.md` |
| **데이터 마이그레이션** | 스키마/데이터 변경 시 | 현재 데이터 구조 파악(코드 탐색 필수), 마이그레이션 계획(대량 데이터 전략), 하위 호환성(API/Kafka/배포순서), 롤백 시나리오, Feature Flag 활용 | `{날짜}_{기능명}_데이터마이그레이션.md` |
| **디자인 검증** | Figma 제공 시 | 디자인↔PRD 불일치, 상태 매핑, 필드 불일치, 인터랙션 BE 지원, 권한별 분기 | `{날짜}_{기능명}_디자인검증.md` |
...
2. **권장 하이브리드 모델**: 여러 제품의 패턴을 조합한 최적 모델 제안
...
| 에이전트 | 분석 항목 |
|---------|---------|
| **아키텍처** | 관련 레포/모듈, Hexagonal 레이어별 코드 위치, API 패턴, 인증/인가 흐름, Kafka 토폴로지, 공유 라이브러리 |
| **데이터** | MySQL 스키마(DB 스키마 레포), Flyway 히스토리, MongoDB, Redis 캐시키, OpenSearch, Read/Write Split, 인덱스/쿼리 패턴 |

## 2-2. TDD 작성 (팀장)

Phase 1 결과 + Phase 1.5 아키텍처 설계 + 2-1 분석 결과를 종합하여 `tdd.md`를 직접 작성한다.

> 템플릿: 사내 Confluence TDD 템플릿 참조
...
**다이어그램 규칙**: Mermaid 사용. flowchart는 항상 `LR` 방향.
...
| 에이전트 | 역할 |
|---------|------|
| **티켓 분할** | 독립 배포 단위로 분할. 1티켓=1관심사. L 이상이면 세분화. 레이어별 분리(Domain→Service→API→Event). 의존관계 도출 |
| **테스트 설계** | 티켓별 TC를 Given/When/Then으로 작성. Unit/Integration/API/E2E 레벨. E2E 카테고리: A(기본), B(AS-IS↔TO-BE 동등성), C(신규 상품) |
...
> Mermaid 가시성 규칙: subgraph 그룹핑, LR 방향 통일, 노드 15개 이하, & 체이닝 금지
