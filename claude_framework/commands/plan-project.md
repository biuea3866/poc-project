---
description: 설계+TDD+티켓 — 정제된 PRD 를 받아 도메인 식별, ADR, 컴포넌트/시퀀스/ERD, TDD 전략, 티켓 분해까지 수행
argument-hint: <prd-output-path>
---

# /plan-project — 설계 → TDD → 티켓

**입력**: $ARGUMENTS (PRD 분석 산출물 경로)

`pipeline-runner` 에이전트가 다음 두 절차를 순서대로 수행한다:

## 1부: 프로젝트 설계 (project-analysis)
...
## 입력 (필수)
- **PRD 분석 산출물** — `outputs/analyze-prd/YYYY-MM-DD-<slug>.md` (Exit Criteria 통과본)
...
폴더 경로: `outputs/plan-project/YYYY-MM-DD-<feature>/`
...
### 1. PRD 정제 (선행 의존)
- `commands/analyze-prd.md` 의 산출물 (`requirements.md`, `acceptance.md`) 사용
...
- 각 티켓 완료 시 PR open → `commands/review-pr.md`
...
- PRD 단계: `commands/analyze-prd.md`
- 프로젝트 분석: `commands/plan-project.md`
