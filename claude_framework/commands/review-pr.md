...
`pipelines/pr-review/PIPELINE.md` 의 절차대로 PR 을 리뷰해줘.
...
### 4. 메타-피드백 트리거 결정
verdict=`request-changes` 면 `pipelines/feedback-loop/PIPELINE.md` 의 트리거 1번 충족 → Stop 훅이 `process-reviewer` 발화 (활성화돼 있을 때).
