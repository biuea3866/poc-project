...
- `.claude/scripts/` — 3-layer 피드백 루프 스크립트 3종 + README (필수)
  - `harness-audit.py` — 전수/diff 감사 (주석 필터, `--diff-files`)
  - `senior-gate.py` — PR 머지 전 보안/구조 Critical 휴리스틱
  - `qa-followup-extract.py` — QA 보고서 "후속 티켓 제안" 표 파서
- `.github/workflows/` — 3-layer 피드백 루프 CI 3종 (필수)
  - `harness-audit.yml` — 매일 03:00 KST 전수 감사 + Issue 자동 발행/갱신
  - `pr-senior-review.yml` — PR open/sync 시 senior-gate + harness-audit(--diff-files) 실행 후 PR 코멘트
  - `qa-followup-tickets.yml` — `docs/qa/*.md` push 시 후속 티켓 자동 발행
- `.analysis/` — 파이프라인 정의 (필수, `feedback-loop/` 포함)
- `CLAUDE.md` — 프로젝트 가이드 (수정 필요)
- `README.md`, `ADOPTION.md` — 문서 (선택)

> `install.sh` 개정 시 위 5개 항목을 복사 대상에 포함하세요. 특히 `.github/workflows/` 3개는 기본 CI 와 충돌하지 않도록 파일명 그대로 추가하는 것으로 충분합니다. 기존 워크플로우가 있으면 overwrite 하지 말고 skip-if-exists 로 동작해야 합니다.
...
## 3-layer Feedback Loop 설치 체크리스트

`install.sh` 가 이식해야 하는 추가 항목 (신규 워크플로우/스크립트):

### 파일 복사 (install.sh 에서 처리)
- [ ] `templates/.claude/scripts/harness-audit.py` → `<project>/.claude/scripts/harness-audit.py` (실행 비트)
- [ ] `templates/.claude/scripts/senior-gate.py` → `<project>/.claude/scripts/senior-gate.py` (실행 비트)
- [ ] `templates/.claude/scripts/qa-followup-extract.py` → `<project>/.claude/scripts/qa-followup-extract.py` (실행 비트)
- [ ] `templates/.claude/scripts/README.md` → `<project>/.claude/scripts/README.md`
- [ ] `templates/.claude/harness-rules.json` → `<project>/.claude/harness-rules.json` (존재 시 skip 또는 `*.new` 로)
- [ ] `templates/.github/workflows/harness-audit.yml` → `<project>/.github/workflows/harness-audit.yml` (skip-if-exists)
- [ ] `templates/.github/workflows/pr-senior-review.yml` → `<project>/.github/workflows/pr-senior-review.yml` (skip-if-exists)
- [ ] `templates/.github/workflows/qa-followup-tickets.yml` → `<project>/.github/workflows/qa-followup-tickets.yml` (skip-if-exists)

### 이식 후 수동 조정 (사용자 몫)
- [ ] `harness-rules.json` 의 `{{BASE_PACKAGE}}` 를 프로젝트 Java/Kotlin 패키지로 치환 (예: `rental.commerce`, `greeting.ats`)
- [ ] `{{TOPIC_PREFIX}}` / `{{TOPIC_CONSTANTS_FILE}}` 를 프로젝트 Kafka 네이밍으로 치환
- [ ] `pr-senior-review.yml` 의 `branches:` 에 실제 base 브랜치 (예: `dev`) 추가 확인
- [ ] `harness-audit.yml` 의 cron 시간대 확인 (기본 18:00 UTC = 03:00 KST)
- [ ] QA 보고서 템플릿에 `## 후속 티켓 제안` 표 섹션 추가 (없으면 Layer 4 무동작)
...
# 2) 스크립트 실행 가능
python3 .claude/scripts/harness-audit.py --format md --fail-on none > /tmp/a.md
python3 .claude/scripts/senior-gate.py --paths src --format md > /tmp/s.md
python3 .claude/scripts/qa-followup-extract.py --format json > /tmp/q.json

# 3) GitHub Actions
gh workflow list | grep -E "(harness-audit|pr-senior-review|qa-followup-tickets)"
```

자세한 운영 절차는 [`.analysis/feedback-loop/PIPELINE.md`](./.analysis/feedback-loop/PIPELINE.md), 건강 상태 점검은 `feedback-loop-guardian` 에이전트 참조.
