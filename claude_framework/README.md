...
- 🛡️ **다층 방어** — PreToolUse 훅(0.5s) → pre-commit → PR Senior Gate(LLM) → nightly 전수 감사
- 🔁 **메타-피드백 루프** — Stop/SubagentStop 훅에서 `process-reviewer` 가 잘못 작성된 prompt(command/skill/rule) 진단. 자동 .md 수정 금지, 제안 파일만 생성 후 사람 승인 PR
- 🔒 **git_upstream_guard** — feature 브랜치 upstream 이 main/dev 로 잘못 설정돼 `git push` 가 보호 브랜치 직행하는 사고를 동적으로 차단

> 설계 결정 + 마이그레이션 단계: [`REFACTOR.md`](./REFACTOR.md)

---

## TL;DR

> 📘 **이식·활성화·일상 사용 통합 가이드**: [`USAGE.md`](./USAGE.md)
> 🏗️ **설계 원칙·안티패턴**: [`REFACTOR.md`](./REFACTOR.md)
