...
# product_noidx 를 채운 뒤 나머지 두 테이블에 그대로 복제한다. 세 테이블의 행이 같아야
...
while [ $start -le $ROWS ]; do
  ...
  cat <<EOF
INSERT INTO product_noidx (id, name, price, category, description, stock_quantity, created_date)
...
while [ $start -le $ROWS ]; do
  ...
  for t in product_composite_noidx product_partitioned_noidx; do
    echo "INSERT INTO $t SELECT * FROM product_noidx WHERE id BETWEEN $start AND $end;"
  done
  start=$((end + 1))
done
...
ANALYZE TABLE product_noidx, product_composite_noidx, product_partitioned_noidx;
EOF
