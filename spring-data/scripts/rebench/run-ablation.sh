...
restart_mysql() {
  ...
  until docker exec -i $CONTAINER mysql -uroot -proot -N -B partition_ablation -e "SELECT 1" >/dev/null 2>&1; do sleep 2; done
}

CASES=(
  "range-plain-idx|$APP/api/rebench/a/plain?$RANGE&orderBy=date"
  "range-plain-noidx|$APP/api/rebench/a/plain-noidx?$RANGE&orderBy=date"
  "range-part-noidx|$APP/api/rebench/a/partitioned-noidx?$RANGE&orderBy=date"
  "agg-plain-idx|$APP/api/rebench/agg/plain?$RANGE"
  "agg-plain-noidx|$APP/api/rebench/agg/plain-noidx?$RANGE"
  "agg-part-noidx|$APP/api/rebench/agg/partitioned-noidx?$RANGE"
)
