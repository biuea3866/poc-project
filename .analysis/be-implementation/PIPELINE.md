...
━━ Phase 3: TDD (기술 설계 문서) 작성 ━━━━━━━━━━━━━━━━━━
    ...
    │
    │  출력: tdd.md
    │    - 템플릿: https://doodlin.atlassian.net/wiki/x/RwHAOw
    │    - 목차:
    │      1. 개요 (배경, 목표, 범위)
...
**분석 항목**:
1. 관련 레포 식별 (greeting-new-back, greeting-api-gateway 등)
...
4. 인증/인가 흐름 (JWT → Gateway → Plan 체크 → RBAC)
5. 관련 Kafka 이벤트 토폴로지
6. 공유 라이브러리 사용 현황 (greeting-proto, greeting-common 등)

### Agent 📊: 데이터 분석가 (Phase 2)
**역할**: 데이터 레이어 현황 파악

**분석 항목**:
1. 관련 MySQL 테이블 스키마 (greeting-db-schema/)
2. 기존 Flyway 마이그레이션 히스토리
3. MongoDB 컬렉션 구조 (사용 시)
4. Redis 캐시 키 패턴
5. OpenSearch 인덱스 매핑 (검색 관련 시)
6. Read/Write Split 고려사항
7. 인덱스 전략 및 쿼리 패턴
...
> 참조 템플릿: https://doodlin.atlassian.net/wiki/x/RwHAOw
...
### 4.4 Kafka 이벤트 설계 (해당 시)
  - 토픽, 메시지 스키마 (Avro), Producer/Consumer
### 4.5 캐시 전략 (해당 시)
...
|---|---------|----------|----------|------|
| A-1 | "~할 수 있다" | 권한 조건 불명확 | A: 전체 허용 / B: 관리자만 | B |

## 2. 누락된 요구사항

| # | 누락 항목 | 필요 이유 | 영향도 | 제안 |
|---|----------|----------|--------|------|
| M-1 | 동시 수정 처리 | 여러 사용자 동시 접근 시 데이터 충돌 | High | Optimistic Lock |
...
## 그리팅 프로젝트 설계 원칙 (축적된 피드백)

> 이 섹션은 실제 프로젝트 진행 중 확정된 설계 원칙이다. 모든 산출물에 반드시 적용한다.
...
| **Aggregator + Domain Server** | Facade(오케스트레이션)는 greeting-aggregator, 도메인 로직은 개별 서비스. HTTP Internal API로 통신 |
| **Facade에 비즈니스 로직 없음** | Facade는 순수 오케스트레이터 (HTTP 호출 순서만). if/else, DB 접근 없음 |
| **Strategy 패턴** | 상품 유형별 처리 등 분기가 필요한 곳은 Strategy 인터페이스 + 구현체. when/if 분기 대신 |
...
| **Admin API 제공** | Retool에서 수동 실행 + 단건 디버깅 가능하도록 엔드포인트 제공 |
...
| **단일 이벤트 패턴** | 상태별 이벤트 타입을 나누지 않음. 단일 이벤트 + 페이로드(status, productType, productCode)로 소비자가 분기 |
| **순서 보장** | 터미널 상태에서 1회만 발행하여 순서 문제 원천 차단 |
| **이벤트 발행 위치** | 도메인 서비스에서만 발행. Aggregator/Facade에서 발행하지 않음 |
...
| **@ConfigurationProperties 미사용** | DB 기반 런타임 제어. Retool에서 배포 없이 on/off |
| **FeatureContext** | `FeatureContext.ALL` (전역) 또는 `FeatureContext.workspace(id)` (워크스페이스별) |
...
| **그리팅 실제 적용 예시** | AS-IS(현재 코드) vs TO-BE(리팩토링 후) + 향후 확장 예시 (코드 변경 없이 가능한 시나리오) |
...
| 규칙 | 상세 |
|------|------|
| **TestContainers 패턴** | greeting-new-back의 `TestContainers` 싱글턴 object + `TestContainersInitializer` + `BaseIntegrationTest` 패턴 준용 |
...
| **MongoDB Primary 유지** | 듀얼라이트 기간 동안 MongoDB가 Source of Truth. MySQL 실패 시 서비스 영향 없음 |
