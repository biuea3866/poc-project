# [Ticket #4-5] DualRead 서비스

## 개요
- TDD 참조: tdd.md 섹션 5.3
- 선행 티켓: #4-1, #4-2, #4-3, #4-4
- 크기: S
...
class DualReadPaymentLogService(
    ...
    fun findByOrderId(orderId: String): PaymentLogDto {
        if (featureFlagService.getFlag(DualWriteFeatureKeys.ReadFromMysqlPayment, FeatureContext.ALL)) {
            return try {
...
class DualReadCreditService(
    ...
    fun getBalance(workspaceId: Int, creditType: String): Int {
        if (featureFlagService.getFlag(DualWriteFeatureKeys.ReadFromMysqlCredit, FeatureContext.ALL)) {
            return try {
