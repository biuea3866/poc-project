# [Ticket #4-3] 크레딧 사용 이력 듀얼라이트 (DualWriteMessagePointLogService)
...
class DualWriteMessagePointLogService(
    ...
    fun save(pointLog: MessagePointLogsOnWorkspace) {
        ...
        val enabled = featureFlagService.getFlag(DualWriteFeatureKeys.DualWriteMessagePointLog, FeatureContext.ALL)
        if (!enabled) return
