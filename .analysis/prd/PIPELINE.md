...
6. 공유 패키지(doodlin-ui 등) 변경 필요 여부
7. 국제화 키 추가 필요 여부
...
- 도메인: business/domain/
- DB: adaptor/mysql/, adaptor/mongo/
- 이벤트: adaptor/kafka/
...
4. MongoDB 컬렉션 변경
5. Redis 캐시 전략 변경
6. Kafka 토픽 추가 (Terraform)
7. OpenSearch 인덱스 변경

**탐색 대상**:
```
- 스키마: greeting-db-schema/
...
**분석 항목**:
1. 외부 API 호출 추가/변경
2. 커뮤니케이션 채널 (메일/SMS/알림톡/Slack) 변경
3. 결제 플로우 변경
4. Job Board 연동 변경
5. Google/AWS 서비스 변경
...
**탐색 대상**:
```
- 통합: greeting-integration/, integration/
- 커뮤니케이션: greeting-communication/, doodlin-communication/
- 결제: greeting_payment-server/
```
