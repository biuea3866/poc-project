...
[INPUT] PR URL 또는 브랜치 + 관련 티켓 번호
    ...
    ▼
━━ Phase 1: 설계 원칙 검증 ━━━━━━━━━━━━━━━━━━━━━━━━━━━
    │  be-implementation/PIPELINE.md의 "그리팅 프로젝트 설계 원칙" 기준으로 검증
    │
    ▼
━━ Phase 2: 아키텍처 검증 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
be-implementation/PIPELINE.md "그리팅 프로젝트 설계 원칙" 9개 카테고리를 자동 검증한다.
...
| Aggregator에 도메인 로직 없음 | aggregator 코드에서 Repository/Entity import 확인 | 존재하면 위반 |

### 1.4 이벤트 검증

| 검증 항목 | 방법 | 위반 판정 |
|----------|------|----------|
| Kafka 토픽명 | 코드에서 `order.event.v1` 확인 | 다른 토픽명이면 위반 |
| 터미널 상태만 발행 | KafkaTemplate.send 호출 지점의 상태 확인 | PENDING/PAID에서 발행하면 위반 |
| aggregator에서 발행 안 함 | aggregator 코드에서 KafkaTemplate 확인 | 존재하면 위반 |
...
| Internal API | TDD 명세 vs 실제 InternalController 엔드포인트 일치 |
...
|---|---------|------|----------|----------|--------|
| V-1 | DB 설계 | COMMENT 필수 | credit_balance.version 컬럼에 COMMENT 없음 | COMMENT '낙관적 락 버전' 추가 | Minor |
...
- [ ] ⚠️ credit_balance.version COMMENT 누락
```
...
- **구체적으로** — "COMMENT 누락" 대신 "credit_balance.version 컬럼에 COMMENT 없음".
