...
/**
 * 완벽하게 독립적이고 통계적으로 유의미한 성능 비교 테스트
 *
 * 특징:
 ...
 * 5. 격리된 환경에서 실행
 */
class IndependentExcelBenchmark {
    ...
    companion object {
        private const val WARMUP_ITERATIONS = 3  // 워밍업 반복 횟수
        private const val MEASUREMENT_ITERATIONS = 5  // 실제 측정 반복 횟수
        private const val GC_WAIT_TIME_MS = 500L  // GC 대기 시간
    }

    @Test
    fun `POI 독립 벤치마크 - 1000건`() {
        println("\n" + "=".repeat(100))
        println("POI (SXSSFWorkbook) 독립 벤치마크 - 1,000건")
        println("=".repeat(100))
        runIndependentBenchmark(1_000, BenchmarkTarget.POI)
    }

    @Test
    fun `EasyExcel 독립 벤치마크 - 1000건`() {
        println("\n" + "=".repeat(100))
        println("EasyExcel 독립 벤치마크 - 1,000건")
        println("=".repeat(100))
        runIndependentBenchmark(1_000, BenchmarkTarget.EASY_EXCEL)
    }

    @Test
    fun `POI 독립 벤치마크 - 10000건`() {
        println("\n" + "=".repeat(100))
        println("POI (SXSSFWorkbook) 독립 벤치마크 - 10,000건")
        println("=".repeat(100))
        runIndependentBenchmark(10_000, BenchmarkTarget.POI)
    }

    @Test
    fun `EasyExcel 독립 벤치마크 - 10000건`() {
        println("\n" + "=".repeat(100))
        println("EasyExcel 독립 벤치마크 - 10,000건")
        println("=".repeat(100))
        runIndependentBenchmark(10_000, BenchmarkTarget.EASY_EXCEL)
    }

    @Test
    fun `POI 독립 벤치마크 - 100000건`() {
        println("\n" + "=".repeat(100))
        println("POI (SXSSFWorkbook) 독립 벤치마크 - 100,000건")
        println("=".repeat(100))
        runIndependentBenchmark(100_000, BenchmarkTarget.POI)
    }

    @Test
    fun `EasyExcel 독립 벤치마크 - 100000건`() {
        println("\n" + "=".repeat(100))
        println("EasyExcel 독립 벤치마크 - 100,000건")
        println("=".repeat(100))
        runIndependentBenchmark(100_000, BenchmarkTarget.EASY_EXCEL)
    }
    ...
    private fun runSingleBenchmark(
        ...
        val file = tempDir.resolve("${target.name}_${System.nanoTime()}.xlsx").toFile()
        val executionTime = measureTimeMillis {
            FileOutputStream(file).use { outputStream ->
                when (target) {
                    BenchmarkTarget.POI -> {
                        val service = ExcelGenerationService()
                        service.generateOrderExcelFromSequence(orders, columnConfig, outputStream)
                    }
                    BenchmarkTarget.EASY_EXCEL -> {
                        val service = EasyExcelGenerationService()
                        service.generateOrderExcelFromSequence(orders, columnConfig, outputStream)
                    }
                }
            }
