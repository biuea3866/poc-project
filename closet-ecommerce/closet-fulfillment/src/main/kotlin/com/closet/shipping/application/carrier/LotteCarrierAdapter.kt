...
class LotteCarrierAdapter(
    ...
    override fun registerShipment(request: ShipmentRegistrationRequest): ShipmentRegistrationResponse {
        ...
        logger.info { "[LOTTE] 배송 등록 요청: orderId=${request.orderId}" }
        val response = restTemplate.postForObject(url, body, Map::class.java)
        val data =
            response?.get("data") as? Map<*, *>
                ?: throw RuntimeException("LOTTE 배송 등록 응답 파싱 실패")
    ...
    override fun trackShipment(trackingNumber: String): TrackingResponse {
        val url = "$mockApiBaseUrl/carrier/lotte/api/v1/tracking/$trackingNumber"
        logger.info { "[LOTTE] 배송 추적 요청: trackingNumber=$trackingNumber" }
        ...
        val data =
            response?.get("data") as? Map<*, *>
                ?: throw RuntimeException("LOTTE 배송 추적 응답 파싱 실패")

        return parseTrackingResponse(data, "LOTTE", trackingNumber)
    }
