...
class RoleExtractionFilterTest {
    ...
    private fun generateToken(memberId: Long, role: String? = null): String {
    ...
    fun `JWT에 role=SELLER claim이 있으면 X-Member-Role=SELLER 헤더를 추가한다`() {
        ...
        val request = MockServerHttpRequest.get("/api/v1/shippings")
    ...
    fun `JWT에 role=ADMIN claim이 있으면 X-Member-Role=ADMIN을 전달한다`() {
        ...
        val request = MockServerHttpRequest.get("/api/v1/admin/users")
            .header(HttpHeaders.AUTHORIZATION, "Bearer $token")
            .build()
        val exchange = MockServerWebExchange.from(request)
