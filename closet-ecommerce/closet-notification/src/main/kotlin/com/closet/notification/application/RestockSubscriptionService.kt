...
class RestockSubscriptionService(
    ...
    /** 재입고 알림 구독 */
    @Transactional
    fun subscribe(memberId: Long, productOptionId: Long): RestockSubscriptionResponse {
    ...
    /** 재입고 알림 구독 취소 */
    @Transactional
    fun unsubscribe(memberId: Long, productOptionId: Long) {
        val subscription = restockSubscriptionRepository
    ...
    fun notifyRestock(productOptionId: Long): Int {
        val subscriptions = restockSubscriptionRepository
            .findByProductOptionIdAndIsNotifiedFalseAndDeletedAtIsNull(productOptionId)

        subscriptions.forEach { subscription ->
            subscription.markNotified()

            val notification = com.closet.notification.domain.Notification.create(
