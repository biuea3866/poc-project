...
CREATE TABLE IF NOT EXISTS saga_execution (
    id              BIGINT          NOT NULL AUTO_INCREMENT  COMMENT 'Saga 실행 PK',
    saga_id         VARCHAR(100)    NOT NULL                 COMMENT 'Saga ID (UUID)',
    order_id        BIGINT          NOT NULL                 COMMENT '주문 ID',
    current_step    VARCHAR(50)     NOT NULL                 COMMENT '현재 단계',
    status          VARCHAR(30)     NOT NULL                 COMMENT 'STARTED/INVENTORY_RESERVED/PAYMENT_APPROVED/COMPLETED/COMPENSATING/FAILED',
    failure_reason  TEXT            NULL                     COMMENT '실패 사유',
    created_at      DATETIME(6)     NOT NULL                 COMMENT '생성일시',
    updated_at      DATETIME(6)     NOT NULL                 COMMENT '수정일시',
    PRIMARY KEY (id),
    UNIQUE INDEX idx_saga_order (order_id),
    INDEX idx_saga_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Saga 실행 상태';
