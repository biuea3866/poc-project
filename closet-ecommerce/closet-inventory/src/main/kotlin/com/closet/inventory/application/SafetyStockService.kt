...
class SafetyStockService(
     ...
     */
    @Transactional
    fun updateSafetyThreshold(inventoryId: Long, threshold: Int): InventoryResponse {
        require(threshold >= 0) { "안전재고 임계값은 0 이상이어야 합니다" }
        val inventory = inventoryRepository.findByIdAndDeletedAtIsNull(inventoryId)
     ...
     */
    @Transactional
    fun registerRestockNotification(memberId: Long, productOptionId: Long): RestockNotificationResponse {
        // 이미 등록 여부 체크
        val exists = restockNotificationRepository.existsByProductOptionIdAndMemberIdAndStatus(
        ...
        val notification = RestockNotification.create(
            productOptionId = productOptionId,
            memberId = memberId,
        )
    ...
    fun expireOldNotifications(): Int {
        val waitingNotifications = restockNotificationRepository.findByProductOptionIdAndStatus(
            productOptionId = 0, // 전체 조회를 위해 별도 쿼리 필요
            status = RestockNotificationStatus.WAITING,
        )
