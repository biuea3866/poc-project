...
class CouponService(
    ...
    @Transactional
    fun issueCoupon(couponId: Long, memberId: Long): MemberCouponResponse {
        // Redis DECR로 선착순 동시성 제어
        val redisKey = couponStockKey(couponId)
        val remaining = redisTemplate.opsForValue().decrement(redisKey)
    ...
    @Transactional
    fun useCoupon(couponId: Long, orderId: Long, memberId: Long): MemberCouponResponse {
        val memberCoupons = memberCouponRepository.findByMemberId(memberId)
        val memberCoupon = memberCoupons.find { it.couponId == couponId }
    ...
    fun validateCoupon(couponId: Long, orderAmount: BigDecimal): CouponValidationResponse {
        val coupon = couponRepository.findById(couponId)
            .orElseThrow { BusinessException(ErrorCode.ENTITY_NOT_FOUND, "쿠폰을 찾을 수 없습니다. id=$couponId") }

        val isValid = coupon.isValid() && orderAmount >= coupon.minOrderAmount
        val discountAmount = if (isValid) {
