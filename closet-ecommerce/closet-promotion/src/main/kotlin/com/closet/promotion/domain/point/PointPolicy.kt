...
class PointPolicy(
    ...
    @Enumerated(EnumType.STRING)
    @Column(name = "grade_type", nullable = false, length = 30, columnDefinition = "VARCHAR(30)")
    val gradeType: GradeType = GradeType.ALL,
    ...
    fun calculatePoint(orderAmount: BigDecimal, grade: GradeType): Int {
        if (!isActive) return 0
        if (gradeType != GradeType.ALL && gradeType != grade) return 0

        return when {
            pointAmount != null -> pointAmount
            pointRate != null -> orderAmount
                .multiply(pointRate)
