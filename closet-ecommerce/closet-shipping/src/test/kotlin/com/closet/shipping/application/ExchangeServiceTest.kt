...
class ExchangeServiceTest : BehaviorSpec({
    ...
    Given("교환 신청 - 단순변심") {
        ...
        When("CHANGE_OF_MIND 사유로 교환 신청") {
            ...
            val policy = ShippingFeePolicy(
                type = "EXCHANGE", reason = "CHANGE_OF_MIND",
                payer = "BUYER", fee = Money.of(6000),
            )
            every { shippingFeePolicyRepository.findByTypeAndReasonAndIsActiveTrue("EXCHANGE", "CHANGE_OF_MIND") } returns Optional.of(policy)
            every { exchangeRequestRepository.save(any()) } answers { firstArg() }

            val response = exchangeService.createExchangeRequest(
                memberId = 1L,
                sellerId = 1L,
    ...
    Given("교환 신청 - 불량") {
        ...
        When("DEFECTIVE 사유로 교환 신청") {
            ...
            val policy = ShippingFeePolicy(
                type = "EXCHANGE", reason = "DEFECTIVE",
    ...
    Given("교환 신청 - 배송 미완료") {
        ...
        When("READY 상태에서 교환 신청") {
            val shipment = Shipment.create(
                orderId = 4L, sellerId = 1L, memberId = 1L,
