package com.closet.external.carrier.logen
...
@RestController
@RequestMapping("/carrier/logen/api/v1")
class LogenController(private val carrierService: CarrierService) {

    @PostMapping("/shipments")
    fun register(@RequestBody request: Map<String, Any>): ResponseEntity<*> {
        val shipment = carrierService.registerShipment("LOGEN", "LG", request)
        return ResponseEntity.ok(resp(shipment))
    }

    @GetMapping("/tracking/{trackingNumber}")
    fun track(@PathVariable trackingNumber: String): ResponseEntity<*> {
        val shipment = carrierService.getTracking("LOGEN", trackingNumber)
