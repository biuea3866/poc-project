package com.closet.external.carrier.lotte
...
@RestController
@RequestMapping("/carrier/lotte/api/v1")
class LotteGlobalController(private val carrierService: CarrierService) {

    @PostMapping("/shipments")
    fun register(@RequestBody request: Map<String, Any>): ResponseEntity<*> {
        val shipment = carrierService.registerShipment("LOTTE_GLOBAL", "LT", request)
        return ResponseEntity.ok(resp(shipment))
    }

    @GetMapping("/tracking/{trackingNumber}")
    fun track(@PathVariable trackingNumber: String): ResponseEntity<*> {
        val shipment = carrierService.getTracking("LOTTE_GLOBAL", trackingNumber)
