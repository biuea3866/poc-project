...
router.post('/v1/payments', basicAuthMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const paymentKey = `toss_payment_key_${uuidv4()}`;
...
router.post('/v1/payments/confirm', basicAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const scenario = getScenario(req.headers['x-mock-scenario'] as string);
    await applyScenario(scenario);
...
router.get('/v1/payments/:paymentKey', basicAuthMiddleware, async (req: Request, res: Response) => {
  try {
...
router.post('/v1/payments/:paymentKey/cancel', basicAuthMiddleware, async (req: Request, res: Response) => {
  try {
    const scenario = getScenario(req.headers['x-mock-scenario'] as string);
    await applyScenario(scenario);
    ...
    const cancels = await paymentService.getCancelsByPaymentId(result.payment.id);
