...
exports.tidToPgToken = exports.router = void 0;
...
const tidToPgToken = new Map();
exports.tidToPgToken = tidToPgToken;
// KakaoAK Auth middleware
function kakaoAuthMiddleware(req, res, next) {
...
router.post('/v1/payment/ready', kakaoAuthMiddleware, async (req, res) => {
    try {
        ...
        const { cid, partner_order_id, partner_user_id, item_name, quantity, total_amount, tax_free_amount, approval_url, cancel_url, fail_url, } = req.body;
        // Validation
        ...
        await payment_service_1.paymentService.createPayment({
            provider: types_1.PgProvider.KAKAO_PAY,
            paymentKey,
...
router.post('/v1/payment/approve', kakaoAuthMiddleware, async (req, res) => {
    try {
        const scenario = (0, scenarios_1.getScenario)(req.headers['x-mock-scenario']);
        await (0, scenarios_1.applyScenario)(scenario);
        if (scenario.body) {
...
router.post('/v1/payment/cancel', kakaoAuthMiddleware, async (req, res) => {
    try {
        const scenario = (0, scenarios_1.getScenario)(req.headers['x-mock-scenario']);
        await (0, scenarios_1.applyScenario)(scenario);
        if (scenario.body) {
        ...
        const paymentKey = `kakao_${tid}`;
        const result = await payment_service_1.paymentService.cancelPayment(paymentKey, '고객 요청', cancel_amount);
        const response = {
            aid: `A${Date.now()}${Math.floor(Math.random() * 1000)}`,
            tid,
