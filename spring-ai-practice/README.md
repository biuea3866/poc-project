...
| 2 | **`ChatModel`** (Ollama 어댑터) | autoconfig |
...
| 12 | **`BeanOutputConverter`** (`chatClient.entity(...)`) | [ChatGatewayService.kt](src/main/kotlin/com/biuea/springai/chat/ChatGatewayService.kt) — `/chat` 응답을 `ChatAnswer` 로 받음 · [ExtractionTool.kt](src/main/kotlin/com/biuea/springai/tool/ExtractionTool.kt) — `ExtractedProduct` |
...
| 경로 | 인증 | 권한 검사 | 비고 |
|---|---|---|---|
| `POST /auth/token` | permitAll | — | MCP 클라이언트 자격증명 → JWT 발급 (OAuth2 client_credentials grant) |
| `GET /sse`, `POST /mcp/**` | **Bearer JWT 필수** | `@RequireScope` + Spring AOP `ToolGuardAspect` | 외부 LLM 호스트 (Claude Desktop / MCP 클라이언트) 진입점 |
| `POST /chat`, `/chat/stream` | permitAll | 없음 | 브라우저 UI 채팅 — 인증 없이 동작 |
| 정적 리소스 / `/actuator/health` | permitAll | — | |

UI 의 "MCP 클라이언트 토큰 발급" 폼은 사용자 로그인이 아니라 시연용 페르소나 선택입니다.
UI 채팅 자체는 인증 없이 작동합니다.

### Structured Output — 모든 채팅 응답을 LLM 호출 결과로 구조화

`/chat` 응답은 `BeanOutputConverter` 로 JSON Schema 가 LLM 프롬프트에 자동 주입되어
...
UI 는 `result.answer` 를 채팅 메시지 본문으로 렌더링하고, 그 아래에 `intent` 메타데이터를 작은 글씨로 함께 표시합니다.
...
| `POST /auth/token` | POST | permitAll | MCP 클라이언트 자격증명 → JWT 발급 (OAuth2 client_credentials) |
